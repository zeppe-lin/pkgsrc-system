...
If the kernel is located somewhere else `grub-mkconfig` will NOT find
it!  For example, `/boot/vmlinuz-3.10.28` is good and
`/boot/linux-3.10.28` would not be found.

2) When dual-booting with Linux installations on other partitions or
drives, `grub-mkconfig` will not find them without the extra program
`os-prober` installed.
