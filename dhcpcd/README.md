...
- Scope: `dhcpcd` should only manage DHCP, not Wi-Fi authentication.
- Purpose: `wpa_supplicant` is run explicitly via
...
- `nolink`
  Don't receive link messages about carrier status.  You should only
...
If you are on a network with DHCPv4 that filters Client IDs based on
...
See [RFC 4361][2] for more information.
...
For some (noncompliant) routers, you will not be able to connect
properly unless you comment the line:
